...
默认值在 `lib/src/core/app_config.dart`。业务 API 可以是 `http` 或 `https`，当前线上默认 `https://blcold.cn/api/`。IM TCP 地址不写死，登录后读取业务端 `im_connect.route.tcp_addr`。

## 当前实时架构

客户端不再接入 Flutter IM SDK，不保留 SDK 封装代码。实时层由 `lib/src/im/business_im_service.dart` 直接实现：

- 登录后调用业务端 `im_connect` 获取 `uid`、`token`、`device_flag`、`device_level`、`tcp_addr`。
- 使用 `Socket.connect(tcp_addr)` 建立 TCP 长连接。
- 按业务端接入的悟空 IM TCP 协议发送 connect 包，并完成 X25519/AES 握手。
- TCP 收到消息后校验 `msg_key`、AES 解密、Base64 解码业务 payload，然后写入 MMKV 消息缓存并通知 UI。
- 收到可持久化消息后发送 `recvack`。
...
通用参数：`appid`、`usertoken`、`device`、`device_flag`、`device_level`、`timestamp`、`sign`、`client_msg_no`、`content_type`。
...
发送时客户端先写入 MMKV 本地“发送中”消息，业务端返回后按同一 `client_msg_no` 合并为“已发送”或“队列中”。接收方和发送方的服务端推送都通过 TCP 长连接实时进入本地缓存。
...
服务层同时暴露本地广播消息流。聊天页和首页会话列表订阅该事件流后直接增量合并消息和会话，TCP 收包、本地发送中、发送确认、发送失败都会立即推送到 UI；重新读取 MMKV 只作为冷启动和补偿同步使用。
