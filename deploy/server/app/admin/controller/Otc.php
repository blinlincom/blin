...
class Otc extends Backend
{
    ...
    public function index()
    {
        if (Request::isAjax()) {
            ...
            $q=Db::name('otc_order')->alias('o')->leftJoin('app a','a.appid=o.appid')->leftJoin('user b','b.id=o.buyer_id and b.appid=o.appid')->leftJoin('user s','s.id=o.seller_id and s.appid=o.appid')->leftJoin('otc_fiat_hold fh','fh.appid=o.appid and fh.order_no=o.order_no')->leftJoin('wallet_asset_hold ah','ah.appid=o.appid and ah.biz_type="otc_order" and ah.biz_no=o.order_no')->field('o.*,a.appname,b.username buyer_username,b.nickname buyer_nickname,s.username seller_username,s.nickname seller_nickname,fh.hold_no fiat_hold_no,fh.amount fiat_hold_amount,fh.status fiat_hold_status,ah.hold_no asset_hold_no,ah.original_amount asset_hold_amount,ah.status asset_hold_status');
    ...
    {
        if(Request::isPost()){$appid=input('appid/d');if($appid<=0)return ResponseHelper::error('APPID不能为空');$now=date('Y-m-d H:i:s');$rate=$this->money(input('ad_deposit_rate','5'));if(bccomp($rate,'0.00',2)<0||bccomp($rate,'100.00',2)>0)return ResponseHelper::error('广告保证金比例必须在0到100之间');$values=['enabled'=>input('enabled/d')===1?1:0,'manual_escrow'=>0,'payment_balance_enabled'=>input('payment_balance_enabled/d')===1?1:0,'payment_alipay_enabled'=>input('payment_alipay_enabled/d')===1?1:0,'payment_wechat_enabled'=>input('payment_wechat_enabled/d')===1?1:0,'payment_bank_enabled'=>input('payment_bank_enabled/d')===1?1:0,'merchant_deposit'=>$this->money(input('merchant_deposit','1000')),'ad_deposit_rate'=>$rate,'payment_timeout_minutes'=>max(5,min(120,input('payment_timeout_minutes/d')?:15)),'max_open_orders'=>max(1,min(20,input('max_open_orders/d')?:5)),'max_merchant_ads'=>max(1,min(100,input('max_merchant_ads/d')?:10)),'risk_notice'=>mb_substr(trim((string)input('risk_notice','')),0,500),'update_time'=>$now];if($values['payment_balance_enabled']!==1)return ResponseHelper::error('当前OTC必须开启平台余额结算');$row=Db::name('otc_config')->where('appid',$appid)->find();if($row)Db::name('otc_config')->where('id',(int)$row['id'])->update($values);else Db::name('otc_config')->insert($values+['appid'=>$appid,'create_time'=>$now]);$this->audit($appid,'config_update','config',(int)($row['id']??0),$row?:[],$values,'更新OTC配置');return ResponseHelper::success('配置已保存');}
        return View::fetch();
    }
