...
class Wallet extends Backend
{
    ...
    {
        if(Request::isPost()){$id=input('id/d');$row=Db::name('wallet_sweep_config')->where('id',$id)->find();if(!$row)return ResponseHelper::error('归集配置不存在');$target=trim((string)input('target_address',''));$resource=trim((string)input('resource_address',''));if($target!==''&&!preg_match('/^T[1-9A-HJ-NP-Za-km-z]{33}$/',$target))return ResponseHelper::error('归集钱包地址格式错误');if($resource!==''&&!preg_match('/^T[1-9A-HJ-NP-Za-km-z]{33}$/',$resource))return ResponseHelper::error('资源钱包地址格式错误');$enabled=input('auto_enabled/d')===1?1:0;if($enabled&&(!$this->chainWorkerReady()||$target===''||$resource===''))return ResponseHelper::error('链上执行服务、归集钱包或资源钱包尚未就绪');Db::name('wallet_sweep_config')->where('id',$id)->update(['target_address'=>$target,'resource_address'=>$resource,'min_sweep_amount'=>$this->assetAmount(input('min_sweep_amount','10')),'auto_enabled'=>$enabled,'signing_enabled'=>$enabled,'update_time'=>date('Y-m-d H:i:s')]);return ResponseHelper::success($enabled?'自动归集已开启':'归集配置已保存，自动执行关闭');}
        ...
        $configs=Db::name('wallet_sweep_config')->alias('s')->leftJoin('otc_asset a','a.id=s.asset_id and a.appid=s.appid')->leftJoin('otc_network n','n.id=s.network_id and n.appid=s.appid')->field('s.*,a.symbol,n.code network_code')->select()->toArray();$addresses=Db::name('wallet_chain_address')->alias('ca')->leftJoin('user u','u.id=ca.user_id and u.appid=ca.appid')->field('ca.*,u.username,u.nickname')->whereNotNull('ca.last_deposit_time')->order('ca.id desc')->select()->toArray();$workerHealth=$this->chainWorkerHealth();View::assign(compact('configs','addresses','workerHealth'));return View::fetch();
    }

    public function asset_withdrawals()
    {
        if(Request::isPost()){try{$action=trim((string)input('action',''));$id=input('id/d');if($action==='approve')ChainTaskService::approveWithdraw($id,$this->adminId());elseif($action==='reject')ChainTaskService::rejectWithdraw($id,$this->adminId(),trim((string)input('reason','')));else return ResponseHelper::error('操作类型错误');return ResponseHelper::success($action==='approve'?'提币审核已通过':'提币已驳回并退回冻结资产');}catch(\Throwable $e){return ResponseHelper::error($e->getMessage());}}
    ...
    private function chainWorkerReady(): bool
    {
        $data=$this->chainWorkerHealth();return !empty($data['ok'])&&!empty($data['chain_worker_enabled'])&&!empty($data['resource_wallet_configured']);
    }

    private function chainWorkerHealth(): array
    {
        $context=stream_context_create(['http'=>['timeout'=>2,'ignore_errors'=>true]]);$raw=@file_get_contents('http://127.0.0.1:9088/health',false,$context);$data=$raw===false?null:json_decode($raw,true);return is_array($data)?$data:['ok'=>false];
    }
    ...
    public function gasfree()
    {
        if(Request::isPost()){
            try{$action=trim((string)input('action','config'));if($action==='whitelist'){$appid=input('appid/d');$userId=input('user_id/d');$enabled=input('enabled/d')===1?1:0;if($appid<=0||$userId<=0)throw new \InvalidArgumentException('应用和用户不能为空');$user=Db::name('user')->where(['appid'=>$appid,'id'=>$userId])->find();if(!$user)throw new \RuntimeException('用户不存在');$row=Db::name('wallet_gasfree_whitelist')->where(['appid'=>$appid,'user_id'=>$userId])->find();$values=['status'=>$enabled,'operator_id'=>$this->adminId(),'update_time'=>date('Y-m-d H:i:s')];if($row)Db::name('wallet_gasfree_whitelist')->where('id',$row['id'])->update($values);else Db::name('wallet_gasfree_whitelist')->insert($values+['appid'=>$appid,'user_id'=>$userId,'create_time'=>date('Y-m-d H:i:s')]);return ResponseHelper::success('灰度名单已更新');}
                $id=input('id/d');$row=Db::name('wallet_gasfree_config')->where('id',$id)->find();if(!$row)throw new \RuntimeException('GasFree配置不存在');$enabled=input('enabled/d')===1?1:0;$auto=input('auto_enabled/d')===1?1:0;$mode=trim((string)input('rollout_mode','off'));if(!in_array($mode,['off','whitelist','all'],true))throw new \InvalidArgumentException('灰度模式错误');$health=$this->chainWorkerHealth();if(($enabled||$auto)&&empty($health['gasfree_credentials_ready']))throw new \RuntimeException('GasFree API凭据尚未在钱包服务配置');if($auto&&!$enabled)throw new \RuntimeException('开启自动归集前必须先启用GasFree');$maxTransfer=$this->assetAmount(input('max_transfer_fee','2'));$maxFirst=$this->assetAmount(input('max_first_fee','4'));$minNet=$this->assetAmount(input('min_net_amount','10'));$rate=bcadd((string)input('max_fee_rate','10'),'0',4);if(bccomp($maxFirst,$maxTransfer,8)<0||bccomp($rate,'0',4)<=0||bccomp($rate,'100',4)>0)throw new \InvalidArgumentException('费用阈值配置不正确');Db::name('wallet_gasfree_config')->where('id',$id)->update(['enabled'=>$enabled,'auto_enabled'=>$auto,'rollout_mode'=>$mode,'max_transfer_fee'=>$maxTransfer,'max_first_fee'=>$maxFirst,'min_net_amount'=>$minNet,'max_fee_rate'=>$rate,'config_version'=>Db::raw('config_version+1'),'update_time'=>date('Y-m-d H:i:s')]);return ResponseHelper::success('GasFree配置已保存');
            }catch(\Throwable $e){return ResponseHelper::error($e->getMessage());}
        }
        if(Request::isAjax()){$limit=max(1,min(100,input('limit/d')?:20));$page=max(1,input('page/d')?:1);$type=trim((string)input('type','account'));if($type==='transfer'){$q=Db::name('wallet_gasfree_transfer')->alias('t')->leftJoin('user u','u.id=t.user_id and u.appid=t.appid')->field('t.*,u.username,u.nickname');}elseif($type==='fee'){$q=Db::name('wallet_gasfree_fee_snapshot');}else{$q=Db::name('wallet_gasfree_account')->alias('g')->leftJoin('user u','u.id=g.user_id and u.appid=g.appid')->field('g.*,u.username,u.nickname');}$count=clone $q;return $this->tableResponse(['rows'=>$q->order('id desc')->page($page,$limit)->select()->toArray(),'total'=>$count->count()]);}
        $configs=Db::name('wallet_gasfree_config')->alias('g')->leftJoin('otc_asset a','a.id=g.asset_id and a.appid=g.appid')->leftJoin('otc_network n','n.id=g.network_id and n.appid=g.appid')->field('g.*,a.symbol,n.code network_code')->select()->toArray();$workerHealth=$this->chainWorkerHealth();$whitelist=Db::name('wallet_gasfree_whitelist')->alias('w')->leftJoin('user u','u.id=w.user_id and u.appid=w.appid')->field('w.*,u.username,u.nickname')->where('w.status',1)->order('w.id desc')->limit(100)->select()->toArray();View::assign(compact('configs','workerHealth','whitelist'));return View::fetch();
    }
