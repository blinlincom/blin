...
final class ChainDepositService
{
    ...
    public static function credit(array $data): array
    {
        ...
        if(!$binding) return ['ignored'=>1,'reason'=>'address_not_managed'];
        ...
        Db::startTrans();try{
            ...
            Db::name('wallet_chain_event')->where('id',$eventId)->update(['process_status'=>'credited','update_time'=>$now]);Db::name('wallet_chain_address')->where('id',(int)$binding['id'])->update(['last_deposit_time'=>$now]);Db::commit();return ['credited'=>1,'user_id'=>(int)$binding['user_id'],'amount'=>$amount,'txid'=>$txid];
